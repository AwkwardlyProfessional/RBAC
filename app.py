...
class BlogPost(db.Model):
    ...
    approved = db.Column(db.Boolean, default=False)  # Add this line for approval status
...
def logout():
    session.clear()  # Clear session or perform logout action
...
def moderator_dashboard():
    # Fetch all blog posts, both approved and unapproved
    unapproved_blogs = BlogPost.query.filter_by(approved=False).all()
    approved_blogs = BlogPost.query.filter_by(approved=True).all()
    ...
    if request.method == 'POST':
        ...
        if blog:
            if action == 'approve':
                blog.approved = True
                blog.review = review if review else 'No review provided'  # Store review if provided
    ...
    return render_template('moderator_dashboard.html', 
                           unapproved_blogs=unapproved_blogs,
